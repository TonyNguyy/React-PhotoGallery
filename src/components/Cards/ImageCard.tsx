...
export const ImageCard = ({id,slug,isSelected, onClick, className=""}: ImageCard) => {
    ...
    ref= {setNodeRef}
    style={style}
    className={twMerge("relative round-lg overflow-hidden border border-gray-300 group z-0 aspect-square object-cover", className)}>
    ...
    className={twMerge("absolute top-2 z-50 left-2 group-hover:opacity-100 transition-opacity duration-500 hover:scale-150", isSelected && "!opacity-100", !isSelected && "opacity-0")}>
        {
        ...
        {/* Image Block Here  */}
         <div className={twMerge("flex items-center justify-center h-full", isSelected && "opacity-60")} >
             <img src={slug} alt="Gallery-Img" className="block h-full w-full object-cover "/>
        </div>
